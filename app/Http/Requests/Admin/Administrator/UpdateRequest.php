...
class UpdateRequest extends FormRequest
{
    ...
    {
        $administratorId = $this->route('administrator')->id;

        return [
            'username' => ['required', 'string', 'max:100', Rule::unique('users', 'username')->ignore($administratorId)],
            'email' => ['required', 'email', 'max:255', Rule::unique('users', 'email')->ignore($administratorId)],
            'phone' => ['nullable', 'string', 'max:20', Rule::unique('users', 'phone')->ignore($administratorId), 'phone:INTERNATIONAL,BD'],
