                    ...
                    axios.get(`{{ route($pageConfig->routes->show, ":id") }}`.replace(':id', agentUnitId))
                        .then(response => {
                            ...
                            if (response.data.success) {
                                ...
                                let shopDepositData = data.shop_deposit;

                                // Populate modal fields
                                let TableData = {
                                    '{{ __("Agent:") }}': agentData.username,
                                    '{{ __("Shop") }}': "Shop: " + shopData.number,
                                    '{{ __("Sell Price:") }}': data.sale_price,
                                    '{{ __("Status:") }}': data.status.name
                                };
                                ...
                                if(shopDepositData !== null){
                                    modal.querySelector('#date').value = shopDepositData.added_date;
