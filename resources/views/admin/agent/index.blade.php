<x-app-layout>
    ...
    <div class="agent-grid-wrapper">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 3xl:grid-cols-4 gap-4 md:gap-5 xl:gap-6">
            ...
            @foreach($items as $item)
                ...
                <!-- Agent Item -->
                <div class="agent-item bg-white rounded-xl border border-slate-200 p-4 lg:p-6 transition-all hover:shadow-lg relative">
                    <div class="agent-header flex items-center gap-4">
                        ...
                        <a href="#" data-agent-id="{{ $item->id }}" class="absolute right-4 top-4 w-10 h-10 bg-[#F0F2F4] hover:bg-themered hover:text-white transition-all rounded-full inline-flex items-center justify-center">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M4.5 20.25H19.5" stroke="currentcolor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M11.9023 6.14453C12.272 8.43071 14.1975 10.1785 16.5872 10.4104" stroke="currentcolor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M13.0839 4.94203L6.02649 12.1402C5.76001 12.4135 5.50213 12.9519 5.45056 13.3247L5.1325 16.0085C5.02075 16.9776 5.74282 17.6403 6.73997 17.4746L9.50791 17.019C9.89473 16.9528 10.4363 16.6794 10.7028 16.3978L17.7602 9.19963C18.9808 7.95714 19.531 6.5407 17.6312 4.8095C15.7401 3.09486 14.3045 3.69954 13.0839 4.94203Z" stroke="currentcolor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                        <div class="info space-y-2">
                            <h6 class="font-bold text-black-500 text-xl">
                                <a href="{{ route('admin.agent.show', $item->id) }}">
                                    {{ __($item->username) }}
                                </a>
                            </h6>
                            <p>
    ...
    <div id="add_new_agent_modal" class="asams-modal fixed inset-0 z-[1024] p-4 max-md:pb-20 xl:p-10 2xl:p-16 bg-pureblack/40 transition-opacity duration-150 ease-linear [&:not(.show)]:opacity-0 [&:not(.show)]:invisible overflow-y-auto">
        <div class="modal-content bg-white w-full max-w-4xl mx-auto p-6 lg:p-8 xl:p-10 border border-slate-400 rounded-2xl relative">
            ...
            <form id="multiStepForm" action="{{ route("admin.agent.store") }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-steps relative">
                    ...
                    <fieldset class="step space-y-6 hidden" data-step="1">
                        ...
                        <div class="mt-10 flex justify-between *:flex *:items-center *:justify-center *:gap-3 *:rounded-lg *:px-6 *:py-3 *:transition-all">
                            <button type="button" class="next-step ml-auto transition-all flex items-center justify-center gap-3 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700">
