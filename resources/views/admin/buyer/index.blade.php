<x-app-layout>
    <div class="flex justify-between flex-wrap items-center mb-6">
        <h4 class="font-medium lg:text-2xl text-xl capitalize text-slate-900 inline-flex items-center gap-3 ltr:pr-4 rtl:pl-4 mb-1 sm:mb-0">
            ...
            {{ __('Buyer') }} <a href="{{ route('admin.buyer.request') }}"> asdf </a>
        </h4>
     ...
     <div class="buyer-grid-wrapper">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-5 xl:gap-6">
            ...
            <div class="buyer sm:flex gap-4 rounded-xl overflow-hidden border border-slate-100 bg-white p-4 lg:p-6">
                ...
                <div class="info sm:flex-1 p-4">
                    <h2 class="text-2xl lg:text-3xl font-bold text-black-500">
                        <a href="{{ route('admin.buyer.show', $item->id) }}">
                            {{ $item?->profile?->full_name ?? $item->username }}
                        </a>
                    </h2>
                    <p class="text-slate-400">
                        {{ __("Since ") }} {{ $item->created_at->format('d M Y')}}
                    </p>
            ...
            <!-- Add New Buyer -->
            <div class="group min-h-[250px] border border-themered bg-[#FFF5F5] rounded-xl text-center transition duration-200 cursor-pointer p-10 text-themered hover:border-black-500 hover:bg-[#F0F2F4] hover:text-black-500">
                <a href="#add_new_buyer_modal" data-popup="modal" class="w-full h-full flex items-center justify-center flex-col">
                    ...
                    <div class="text-xl font-medium">{{ __('New Buyer')}}</div>
                </a>
            </div>
        </div>
    </div>
    ...
    <div id="add_new_buyer_modal" class="asams-modal fixed inset-0 z-[1024] p-4 max-md:pb-20 xl:p-10 2xl:p-16 bg-pureblack/40 transition-opacity duration-150 ease-linear [&:not(.show)]:opacity-0 [&:not(.show)]:invisible overflow-y-auto">
        <div class="modal-content bg-white w-full max-w-4xl mx-auto p-6 lg:p-8 xl:p-10 border border-slate-400 rounded-2xl relative">
            ...
            <form id="multiStepForm" action="{{ route('admin.buyer.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            loadSelect2Dropdown("agent", "agent_id", {}, "Select agent");
            loadSelect2Dropdown("buyer-type", "buyer_type_id", {}, "Select buyer type");
            ...
            loadSelect2Dropdown("status", "status_id", {type: "buyer"}, "Select status");
