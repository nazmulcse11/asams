<x-app-layout>
    <div>
        ...
        <form id="multiStepForm" action="{{ route('admin.console.property.update', $item->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-steps relative">
                    ...
                    <div class="field-body grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <select name="property_type_id" id="property_type_id" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg">
                                    @foreach($types as $type)
                                        <option @if($item->property_type_id == $type->id) selected @endif value="{{ $type->id }}">{{ $type->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        @if($init?->enable_contact_info)
                            <div class="field-item space-y-2">
                                ...
                                <input type="text" name="contact_person" id="contact_person" value="{{ old("contact_person", $item->contact_person) }}" placeholder="Gerard Martin" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <input type="text" name="contact_designation" id="contact_designation" value="{{ old("contact_designation", $item->contact_designation) }}" placeholder="manager, director" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <input type="email" name="contact_email" id="contact_email" value="{{ old("contact_email", $item->contact_email) }}" placeholder="gerard.martin@gmail.com" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <input type="tel" name="contact_number" id="contact_number" value="{{ old("contact_number", $item->contact_number) }}" placeholder="01712 345678" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                    ...
                    <div class="field-body grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        @if($init->enable_address_info)
                            <div class="field-item space-y-2 sm:col-span-2 lg:col-span-4">
                                ...
                                <input type="text" name="address_line1" id="address_line1" value="{{ old("address_line1", $item->addresses?->first()?->address_line1) }}" placeholder="Address Line 1" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                            ...
                            <div class="field-item space-y-2 sm:col-span-2 lg:col-span-4">
                                ...
                                <input type="text" name="address_line2" id="address_line2" value="{{ old("address_line2", $item->addresses?->first()?->address_line2) }}" placeholder="Address Line 2" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg"  />
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <select name="country_id" id="country_id" data-old="{{ old('country_id', $item->addresses?->first()?->country_id) }}" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg"></select>
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <input type="text" name="zip_code" id="zip_code" value="{{ old("zip_code", $item->addresses?->first()?->zip_code) }}" placeholder="Zip Code" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required>
                        ...
                        @if($init->enable_gmaps)
                            ...
                            <span class="absolute left-4 top-1/2 -translate-y-1/2">
                                <svg width="16" height="20" viewBox="0 0 16 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <input type="number" inputmode="numeric" name="latitude" value="{{ old('latitude', $item->latitude) }}" placeholder="Map Latitude" min="0" step=".01" class="w-full text-sm font-light py-4 px-4 pl-12 bg-white border rounded-lg"" />
                                </div>
                            ...
                            <svg width="16" height="20" viewBox="0 0 16 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <path d="M8 13.25C11.4518 13.25 14.25 10.4518 14.25 7C14.25 3.54822 11.4518 0.75 8 0.75C4.54822 0.75 1.75 3.54822 1.75 7C1.75 10.4518 4.54822 13.25 8 13.25ZM8 13.25V16.25" stroke="#8997A9" stroke-width="1.5" stroke-linecap="round"/>
                                    ...
                                    <input type="number" inputmode="numeric" name="longitude" value="{{ old('longitude', $item->longitude) }}" placeholder="Map Longitude" min="0" step=".01" class="w-full text-sm font-light py-4 px-4 pl-12 bg-white border rounded-lg"/>
                                </div>
                ...
                <!-- Step 3 -->
                    ...
                    <div class="form-body grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 my-10">
                        @if($init?->enable_area)
                            <div class="field-item space-y-2">
                                ...
                                <input type="number" inputmode="numeric" name="length" id="length" value="{{ old('length', $item->length)}}" placeholder="250" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <input type="number" inputmode="numeric" name="wide" id="wide" value="{{ old('wide', $item->wide)}}" placeholder="220" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                            ...
                            <div class="field-item space-y-2">
                                ...
                                <input type="number" name="total_area" id="total_area" value="{{ old('total_area', $item->total_area)}}" placeholder="55,500" inputmode="numeric" class="w-full text-sm font-light py-4 px-4 bg-white border rounded-lg" required />
                        ...
                        @if($init?->img_video)
                            <div class="field-item space-y-2">
                                ...
                                <input type="file" inputmode="numeric" name="images[]" id="images" multiple  class="w-full border border-gray-300 rounded-lg p-2 bg-white file:mr-4 file:p-2 file:rounded-lg file:border-0 file:bg-red-600 file:text-white"/>
                            ...
                            <span class="absolute left-4 top-1/2 -translate-y-1/2">
                                <svg width="16" height="20" viewBox="0 0 16 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <input type="url" name="video" id="video" value="{{ old('video', $item->video) }}" placeholder="https://www.youtube.com/videoID" class="w-full text-sm font-light py-4 px-4 pl-12 bg-white border rounded-lg"/>
                                </div>
                    ...
                    <div class="mt-6 flex justify-between *:flex *:items-center *:justify-center *:gap-3 *:rounded-lg  *:px-6 *:py-3">
                        <button type="submit" class="bg-red-600 text-white finish-step">Update</button>
                    </div>
                </fieldset>
            </div>
        </form>
