        ...
        <div class="card">
            <header class=" card-header noborder">
                <div class="justify-end flex gap-3 items-center flex-wrap">
                    {{-- Create Button start--}}
                    @can('role create')
                        <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2 !px-3" href="{{ route('admin.console.role.admin.create') }}">
                    ...
                    <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2.5" href="{{ route('admin.console.role.admin.index') }}">
                        <iconify-icon icon="mdi:refresh" class="text-xl "></iconify-icon>
                    </a>
                </div>
                <div class="justify-center flex flex-wrap sm:flex items-center lg:justify-end gap-3">
                    <div class="relative w-full sm:w-auto flex items-center">
                        <form id="searchForm" method="get" action="{{ route('admin.console.role.admin.index') }}">
                ...
                <div class="overflow-x-auto -mx-6">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                                    ...
                                    <tr class="border border-slate-100 dark:border-slate-900 relative">
                                        ...
                                        <td class="table-td">
                                            <div class="action-btns space-x-2 flex">
                                                {{-- show --}}
                                                    <a class="action-btn" href="{{ route('admin.console.role.admin.show', $role) }}" x-data="{ tooltip: 'View' }" x-tooltip="tooltip">
                                                        <iconify-icon icon="ph:eye-light"></iconify-icon>
                                                    </a>
                                                {{-- Edit --}}
                                                    <a class="action-btn" href="{{ route('admin.console.role.admin.edit', ['role' => $role]) }}">
                                                        <iconify-icon icon="uil:edit"></iconify-icon>
                                                    </a>
                                                {{-- delete --}}
                                                    <form id="deleteForm{{ $role->id }}" method="POST" action="{{ route('admin.console.role.admin.destroy', $role) }}" class="cursor-pointer">
                                                        @csrf
                                                        @method('DELETE')
                            ...
                            <x-table-footer :per-page-route-name="'admin.console.role.admin.index'" :data="$roles" />
                        </div>
                    </div>
                </div>
            </div>
