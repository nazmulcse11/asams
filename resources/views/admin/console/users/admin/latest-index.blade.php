<x-app-layout>
    <div class="flex justify-between flex-wrap items-center mb-4 xl:mb-6">
        ...
        <div class="flex sm:space-x-4 space-x-2 sm:justify-end items-center rtl:space-x-reverse">
            <a href="{{ route('admin.console.user.admin.create') }}" class="text-themered hover:bg-themered hover:text-white transition-all bg-[#FFF0F1] text-sm px-5 py-3 rounded-md font-semibold flex-1 inline-flex items-center justify-center">
    ...
    <div class="user_wrapper grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 xl:gap-6 items-start">
        ...
        <div class="item p-4 xl:p-6 rounded-2xl border border-slate-100 bg-white">
            <div class="thumbnail mb-6 relative">
                <a href="{{ route('admin.console.user.admin.show', $item->id ?? '') }}">
            ...
            <div class="mt-6 flex gap-3 *:text-sm *:py-2.5 *:rounded-md *:font-semibold *:flex-1 *:inline-flex *:items-center *:justify-center *:transition-all">
                <a href="{{ route('admin.console.user.admin.show', $item->id ?? '') }}" class="border border-themered text-themered bg-[#FFF0F1] hover:bg-themered hover:text-white">
                    <iconify-icon class="text-xl ltr:mr-2 rtl:ml-2 font-light" icon="heroicons-outline:eye"></iconify-icon>
                    User Details
                </a>
                <a href="{{ route('admin.console.user.admin.edit', $item->id ?? '') }}" class="border border-gunmetal themered text-gunmetal bg-white hover:bg-gunmetal hover:text-white">
