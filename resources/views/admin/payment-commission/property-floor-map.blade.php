        ...
        <div class="card">
            <div class="card-header noborder justify-end flex gap-3 items-center flex-wrap mb-4 mx-4">
                <div class="input-area">
                    <select name="property" class="form-control" id="propertyDropdown" data-old="{{ old('property', request()->get('property')) }}" readonly="readonly"></select>
    ...
    @push("scripts")
        ...
        <script type="module">
            document.addEventListener("DOMContentLoaded", function () {
                    ...
                    onElementsLoaded(".unit", (units) => {
                        units.forEach(unit => {
                            unit.addEventListener('click', function (e) {
                                ...
                                if(!plan.isEditMode){
                                    window.open("{{ route('admin.shop.show', ':id') }}".replace(":id", unitId));
                                }
                            }); // end click
                        }); // end foreach
                ...
                $("#propertyDropdown").select2({ disabled: true });
            });
        </script>
    @endpush
</x-app-layout>
