<div id="shop-card-4" class="shop-item bg-white rounded-2xl overflow-hidden border border-slate-100 flex flex-col">
    <div class="thumbnail relative">
        <a href="{{ route('employee.shop-details') }}">
            <img src="{{ asset('/images/pic1.png') }}" alt="101" class="min-h-[240px] xl:min-h-[290px] max-h-[290px] w-full object-cover bg-gray-100">
        </a>
    </div>
    <div class="px-4 py-5 xl:px-6 flex-1 flex flex-col gap-5">
        ...
        <div class="finance_states mt-auto">
            <table class="w-full border border-slate-200 rounded-xl border-collapse table-auto">
                ...
                <tr class="*:bg-[#F8F9FB] *:border *:border-slate-200 *:px-1 *:py-1 *:text-xs *:leading-4 *:text-center">
                    <td>
                        Sale Price <br>
                        <b class="text-pureblack">৳{{ $shop->buyerShop?->sell_amount }}</b>
                    </td>
                    <td>
                        Security Money <br>
                        <b class="text-pureblack">৳{{ $shop->buyerShop?->agentShop?->security_deposit }}</b>
                    </td>
                    <td>
                        Reserve Until <br>{{ $shop->buyerShop?->agentShop?->created_at->addDays((int) $shop->buyerShop?->agentShop?->duration)->format('d M Y') }}</b>
                    </td>
                    <td>
                        Commission <br>
                        <b class="text-pureblack">@if($shop->buyerShop?->agentShop?->commission_type == 'amount') ৳ @endif {{ $shop->buyerShop?->agentShop?->commission }} @if($shop->buyerShop?->agentShop?->commission_type == 'percentage') % @endif</b>
                    </td>
                </tr>
                </tbody>
            </table>
        ...
        <div class="!mt-1 flex gap-2 *:text-sm *:py-2 *:rounded-md *:font-semibold *:flex-1 *:inline-flex *:items-center *:justify-center *:gap-1.5 *:transition-all">
            ...
            @if(($shop->pendingRequests()?->isNotEmpty() && getCurrentGuard() == "employee") || ($shop->verifyedRequests()?->isNotEmpty() && getCurrentGuard() == "admin"))
                <button id="view-sale-proposal-{{ $shop->id }}" data-shop-id="{{ $shop->id }}" data-popup="modal" class="view-sale-proposal-{{ $shop->id }} view-sale-proposal text-white bg-themered border border-themered hover:bg-gunmetal hover:border-gunmetal">
                    View Request
                </button>
            @endif
        </div>
