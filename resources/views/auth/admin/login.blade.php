<x-website-layout>
    <div class="authentication pt-40 pb-20 md:pt-52 md:pb-24 xl:pt-64 xl:pb-44 bg-cover bg-bottom" style="background-image: url({{ asset('/images/website/authentication-bg.png') }});">
        <div class="container">
            ...
            @env('local')
                <x-login-link redirect-url="{{ route('admin.dashboard') }}" guard="admin" label="Log in" />
            @endenv

                <div class="flex justify-center mt-10 space-x-4">
                    <a href="{{ route('admin.login') }}"    class="disabled px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 pointer-events-none opacity-50 cursor-not-allowed" aria-disabled="true">Admin</a>
                ...
                <form action="{{ route('admin.login') }}" method="post" class="bg-white shadow-xl rounded-2xl xl:rounded-3xl p-6 sm:p-10 lg:p-14 max-w-xl mx-auto">
                @csrf
                ...
                <div class="form-body space-y-5 my-10">
                    <div class="field-item space-y-2">
                        ...
                        <input type="text" name="email" id="email" placeholder="admin@asams.com" value="{{ old('email') }}" class="w-full">
                    ...
                    <div class="field-item flex items-center justify-between gap-3">
                        ...
                        <a href="{{ route("admin.password.request") }}" class="text-themered font-semibold text-end max-lg:text-sm">
                            Forgot Passowrd?
                        </a>
                    </div>
                </div>
