<x-app-layout>
    <div class="flex justify-between flex-wrap items-center mb-6">
        <h4 class="font-medium lg:text-2xl text-xl capitalize text-slate-900 inline-flex items-center gap-3 ltr:pr-4 rtl:pl-4 mb-1 sm:mb-0">
            ...
            {{ __('Buyer') }}
            @php $buyerEnrolmentCount = getCurrentAuthenticatedUser()->unreadnotifications()->where('type', [\App\Notifications\BuyerEnrollmentNotification::class])->count(); @endphp
            @if($buyerEnrolmentCount > 0)
                <a href="{{ route('employee.buyer.request') }}"
                    id="new_added_buyer"
                    ...
                    <span class="inline-block text-white bg-green-600 rounded-full px-2 py-1.5 text-xs leading-none">{{ $buyerEnrolmentCount }}</span>
                    New Buyer
                </a>
            @endif
        </h4>
                ...
                <div class="buyer sm:flex gap-4 rounded-xl overflow-hidden border border-slate-100 bg-white p-4 lg:p-6">
                    ...
                    <div class="info sm:flex-1 p-4">
                        <h2 class="text-2xl lg:text-3xl font-bold text-black-500">
                            <a href="{{ route('employee.buyer.show', $item->id) }}">
